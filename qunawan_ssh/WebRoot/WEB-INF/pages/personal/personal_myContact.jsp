	...
	<div class="content_left_two">
		...
		<div class="content_right_pa">
			...
			<div class="content_right bordernone " id="usually_tourist_content">
				...
				<div class="tourist">
					<form id="contactForm" name="contactForm" action="contact!addcontact" method="post">
						<ul>
				...
				<div class="Pages"> <!-- id="contactPages" --> 
					<c:if test="${cur!=1 }">
						<a href="contact!showContact?page=${cur-1}" title="上一页" class="PrevPage">上一页</a>
					</c:if>
					<c:if test="${cur==1 }">
					...
					<c:forEach var="i" begin="1" end="${pageCount}">
						<c:if test="${i==cur}">
								<a href="contact!showContact?page=${i}" class="pagesel" style="background-color:#DF1A7A;
							color:white" id="orderPage" onclick="cur(${i})">${i}</a>
						</c:if>
						<c:if test="${i!=cur}">
								<a href="contact!showContact?page=${i}" class="pagesel" 
							 id="orderPage" onclick="cur(${i})">${i}</a>
						</c:if>
					</c:forEach>
					<c:if test="${cur!=pageCount }">
						<a href="contact!showContact?page=${cur+1}" title="下一页" class="NextPage" onclick="next()">下一页</a>
					</c:if>
					<c:if test="${cur==pageCount }">

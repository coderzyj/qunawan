			...
			<div class="content_right" id="personal_detail_content">
					...
					<form name="personForm" method="post"
						...
						<ul>
							<li><span style="margin-left: 30px" class="redcolor"
								...
								id="err_msg">${requestScope.err_msg}<s:debug></s:debug></span></li>
					
							<li><label> 昵称：</label> <input type="text"
								placeholder="请输入昵称" id="myname" name="user.name"
								value="${sessionScope.user.name}" class="input_style" /> <span
							...
							<li><label> <span class="redcolor">*</span>邮箱：
								...
								value="${sessionScope.user.email}" class="input_style" onBlur="checkEmail()" />
								<span id="email_msg"></span></li>
							<li><label> 真实姓名： </label> <input type="text"
								placeholder="请输入真实姓名" id="realname" value="${sessionScope.user.realName }"
								name="user.realName" class="input_style" /> <span id="realname_msg"></span></li>
							<c:choose>
								<c:when test="${empty sessionScope.user.birthday.month}">
								...
								<c:when test="${empty sessionScope.user.birthday.year}">
									<li><label> 生日：</label> <select class="sel_year" rel=""
										name="sel_year"></select> <select class="sel_month" rel=""
										name="sel_month"></select> <select class="sel_day" rel=""
										name="sel_day"></select></li>
								</c:when>
								<c:when test="${empty sessionScope.user.birthday.date}">
									<li><label> 生日：</label> <select class="sel_year" rel=""
										name="sel_year"></select> <select class="sel_month" rel=""
										name="sel_month"></select> <select class="sel_day" rel=""
										name="sel_day"></select></li>
								...
								<c:otherwise>
									<li><label> 生日：</label> <select class="sel_year"
										rel="<fmt:formatDate value="${sessionScope.user.birthday}" pattern="yyyy" />"
										name="sel_year"></select> <select class="sel_month"
										rel="${sessionScope.user.birthday.month+1}" name="sel_month"></select> <select
										class="sel_day" rel="${sessionScope.user.birthday.date}" name="sel_day"></select></li>
								</c:otherwise>
							</c:choose>
							...
							<li><label> 地区： </label> <select id="province"
								...
								onBlur="checkSelect()">
									<c:choose>
										<c:when test="${!empty sessionScope.user.city}">
											<option value='${sessionScope.user.city.id}'>${sessionScope.user.city.name}</option>
										</c:when>
										<c:otherwise>
									...
									<c:forEach items="${provinces }" var="province">
										...
										${city.id==province.id?'selected':''}>${province.name}</option>
									</c:forEach>
							</select> <select id="select_city" name="city" class="city cityclass"
								onBlur="checkSelectCity()">
									<option value='${sessionScope.user.city.id}' selected>${sessionScope.user.city.name}</option>
				...
				<div class="head_detail_content">
					<form name="headForm" method="post"
						action="crop!judge?pictureType=headPic"

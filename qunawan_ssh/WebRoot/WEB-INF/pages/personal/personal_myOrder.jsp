		...
		<div class="content_right_pa">
			...
			<div class="content_right bordernone" id="my_re_list_content">
				...
				<div class="re_my_detail">
							...
							<c:forEach items="${orderFormList}" var="item" varStatus="varStatus">
								...
								<tr>
									...
									<td>
										...
										<span class="text_color span_width"><a  href="order!showDetailorder?order_id=${ item.orderid}" target="view_window">订单详情</a></span>
									</td>
					...
					<c:if test="${cur!=1 }">
						<a href="order!showMyorder?page=${cur-1}" title="上一页" class="PrevPage">上一页</a>
					</c:if>
					<c:if test="${cur==1 }">
					...
					<c:if test="${cur!=pageCount }">
						<a href="order!showMyorder?page=${cur+1}" title="下一页" class="NextPage" onclick="next()">下一页</a>
					</c:if>
					<c:if test="${cur==pageCount }">

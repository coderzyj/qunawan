		...
		<div class="body_bg" style="padding-bottom: 30px;">
			...
			<div class="navigation">
				<span class="font_trip_title"> <a
					...
					href="search.jhtml?triptype=${detail.sequence.value}">${detail.sequence.value}</a>
					> <a class="cur">${detail.STitle }</a>
				</span>
			</div>
			<!-- 导航栏【结束】 -->
					...
					<div class="DB_gallery">
						...
						<div class="DB_thumSet">
							<ul class="DB_thumMove">
								...
								<c:forEach items="${detail.trippictures}" var="t_pic">
									<li><a
					...
					<div class="overview-protect-top clearfix">
						<div class="product-basic-info">
							...
							<div class="product-no clearfix">
								...
								<div class="put-right">
									<span class="val">${detail.goodRate}%</span>
								</div>
							</div>
						</div>
					</div>
						...
						<div class="preorder-input-group chrome-hack-1">
							...
							<div class="trip-time" id="trip-time">
								<input class="preorder-start-time" id="start_time" type="text"
									...
									onclick="AjaxTime(event);" name="date" readonly>
							</div>
						</div>
					...
					<div class="product-detail-instance" id="product_detail">
						...
						<div class="detail-instance-body">
							...
							<div class="product-point ">
								<c:if test="${detail.details.size() > 0}">
									<span> 
										<img alt="" src="img/scenic.jpg" height="69px;">
									</span>
									<br> 
										<c:forEach items="${detail.details}" var="detailss" >
											${detailss.place}
										</c:forEach>
								...
								<div class="detail-instance-body  padding0 clearfix">
									<div class="travel-all">
										...
										<ul id="J_scrollnav1">
											...
											<c:forEach items="${detail.schedules}" var="schedual" >
												<c:forEach end="${schedual.dateses.size()}" begin="1" var="i">
																...
																<div class="left">
																	<ul>
																		...
																		<li class="clearfix bus">
																			...
																			<div>
																				<p title="管家加州派酒店（地址：成都市金牛区二环路北一段14号）">上车点：
																				
																					<c:forEach items="${detail.schedules}" var="sc">
																						${sc.traffic.goPoint }
																					</c:forEach>
																					</p>
																				<p>
																					发车时间：<span class="bus-time">
																						<c:forEach items="${detail.schedules}" var="sw">
																							${sw.traffic.goTime }
																						</c:forEach>
																					</span>
																				</p>
																...
																<div class="left">
																	<ul>
																		<li class="clearfix bus">
																			...
																			<div>
																				<p title="九寨沟沟口">上车点：<c:forEach items="${detail.schedules}" var="sc">
																						${sc.traffic.returnPoint }
																					</c:forEach></p>
																				<p>
																					发车时间：<span class="bus-time"><c:forEach items="${detail.schedules}" var="sw">
																							${sw.traffic.returnTime }
																						</c:forEach></span>
																				</p>
																			</div>
																		</li>
									...
									<div class="travel-days">
										...
										<c:forEach items="${detail.schedules}" var="schedual">
											<c:forEach items="${schedual.dateses }" var="date">
												<div class="travel-case">
												<div class="travel-title clearfix">
									...
									<div class="com-null">
										<div class="com-count">
											<i class="percentum f60" data-mark="dynamicNum"
												data-level="95">${detail.goodRate}</i><span class="f60">%</span>
